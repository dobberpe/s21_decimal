...
int s21_from_float_to_decimal(float src, s21_decimal *dst) {
  ...
  if (isinf(src) || isnan(src) ||
      fabsf(src) > 79228157791897854723898736640.0f ||
      (src != 0.0 &&
       fabsf(src) <
...
int exp_from_str(char *f_str) {
  ...
  int i = -1;
  while (f_str[++i] && !exp)
    if (f_str[i] == 'e') exp = strtol(f_str + i + 1, NULL, 10);
...
int s21_from_decimal_to_int(s21_decimal src, int *dst) {
  ...
  if (!dst || decimal_validation(&src))
    ...
    if (int_part.bits[1] || int_part.bits[2] ||
        s21_is_greater(int_part, int_max) || s21_is_less(int_part, int_min)) {
      *dst = 0;
      res = 1;
    } else {
      *dst = int_part.bits[0];
